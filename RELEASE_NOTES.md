...
- **piper-plus 0.7.0 でも本番ビルドが通るようになったよ〜!🛠️🎙️** — 旧版向けパッチが 0.7.0 のソース差分に適用されず、Turbopack が runtime WASM import を `Can't resolve <dynamic>` で止めていた問題を修正っ✨ 現行版に合わせてパッチを最小化・再生成し、lockfile の patch hash も同期しましたわ〜🛡️

- **Web Push の購読 token をログからしっかりガードしたよ〜!🔒📨** — Push 送信失敗時に購読 endpoint 全体を記録していた処理を provider origin だけの構造化ログへ変更して、ログから capability URL が漏れて不正通知に悪用されるリスクを防いだよっ✨ 回帰テストで endpoint の path/token がログへ含まれないことも固定しましたわ〜🛡️

- **公開年月で記事をピンポイント検索できるようになったよ〜!📅🔎** — 高度検索に`published:`フィールドを追加して、`published:2026-04`みたいに記事の公開日時だけを狙って絞り込めるようになったよっ🎀 ISO日時の一部指定はもちろん、フレーズ検索や`-published:`の除外もいつもの構文でOK〜✨ 構文・対象限定・フレーズ／否定・未設定安全性を4つのテストで固定しましたわ〜🛡️

- **JSON Feedの空っぽ本文を次の候補で救えるようになったよ〜!📰🛟** — `content_html`が空なら`content_text`、それも空なら`summary`へ進んで、有効な本文を取りこぼさなくなったよっ🎀 中身のあるHTMLは今までどおり最優先で、全部空なら空本文のままだから互換性もばっちり〜✨ 2段フォールバック・HTML優先・全候補空を4つのテストで固定しましたわ〜🛡️

- **JSON Feedの日付がちょい壊れてても更新日で救えるようになったよ〜!📅🛟** — `date_published`が空だったり不正な日付だったりしても、有効な`date_modified`があれば記事日時として賢くフォールバックできるようになったよっ🎀 正しい公開日は今までどおり最優先、両方ダメなら`null`の安心設計〜✨ 優先・不正・空・全候補不正の4つのテストで固定しましたわ〜🛡️

- **JSON Feedの画像フォールバックが安全でしぶとくなったよ〜!🖼️🛡️** — `image`や`banner_image`が危険なURLでも、後ろに安全な画像候補があれば優先順を守ってちゃんとサムネに採用できるようになったよっ🎀 画像attachmentも危険な候補を飛ばして次へ進み、全部ダメなら今までどおり空で安心〜✨ 上位2段・attachment連続候補・全候補危険を3つのテストで固定しましたわ〜🧭

- **JSON Feedの外部記事リンクがもっとタフになったよ〜!🔗🛡️** — 記事の`url`が空だったり危険なスキームだったりしても、安全な`external_url`があればリンクとして賢く救済できるようになったよっ🎀 有効な`url`は今までどおり最優先、両方危険ならリンクを空にする安心設計〜✨ 空・危険・全候補危険の3つのテストでフォールバックと安全性を固定しましたわ〜🧭

- **ちょいクセJSON Feedでも外部記事を見失わなくなったよ〜!🧭📰** — 記事の`id`と`url`がなくても`external_url`があれば、リンクだけじゃなくGUIDにも使って別記事をきっちり識別できるようになったよっ🎀 標準どおりの`id`や`url`は今までの優先順位をそのままキープ〜✨ 単独フォールバック・優先順位2段・全候補なしを4つのテストで固定しましたわ〜🛡️

- **Readwiseへ記事の本当の著者名を持ち出せるようになったよ〜!✍️📚** — メモ付き記事のCSVエクスポートで、記事に著者情報があれば`Author`列へ優先して入るようになったよっ🎀 著者が空なら今までどおりフィード名へ戻るから既存データも安心、カンマや引用符入りの名前もCSVエスケープでばっちり〜✨ 著者優先・フォールバック・未解決・特殊文字を4つのテストで固定しましたわ〜🛡️

- **配信元GUIDで記事をピンポイント検索できるようになったよ〜!🪪🔎** — 高度検索に`guid:`フィールドを追加して、RSS・Atom・JSON Feedが持つ元の識別子から記事を探せるようにしたよっ🎀 大文字小文字を気にせず、フレーズ検索や`-guid:`の除外もいつもの構文でOK〜✨ 構文・フィールド分離・フレーズ／否定・未設定安全性を4つのテストで固定しましたわ〜🛡️

- **記事JSONバックアップから配信元までたどれるようになったよ〜!🧭💾** — 記事ごとの`guid`と購読元の`feedUrl`を追加して、記事URLとフィードURLをきっちり分けたよっ🎀 対応するフィードがない場合も`null`で安全に扱えるよう、3つのテストでばっちりガード済み〜✨

- **Atomの共同執筆記事で著者全員が見えるようになったよ〜!✍️👥** — entryに複数の`author`が並ぶAtomフィードでも、名前を元の順番で`, `区切りにして記事へ取り込めるようになったよっ🎀 entry側に著者がないときはfeed全体の複数著者も賢く継承し、空名はスルー、単一著者の表示はそのままキープ〜✨ entry・feed継承・空値・単一互換を回帰テスト3件で固定しましたわ〜🛡️

- **Markdown保存でも記事メタデータをまるっと持ち出せるようになったよ〜!📝🧩** — Markdown保存・コピー・Obsidian連携のfrontmatterに、`dc:source`や`business_form`みたいな追加情報を`key`/`value`配列で順番どおり出力っ🎀 任意キーをYAML項目へ直書きせず、キーと値の両方に既存エスケープを通すから、コロン・引用符・改行入りでも安心だよ〜🛡️ 構造・順序・特殊文字・未設定省略を純粋関数テスト3件で固定しましたわ〜✨

- **記事JSONバックアップにカスタムメタデータも付いてくるようになったよ〜!🧩💾** — ブックマーク・後で読む・コレクションをJSON保存すると、`dc:source`や`business_form`、`language`みたいなRSS・Atom・JSON Feed由来の追加情報も順番そのままで持ち出せちゃうっ🎀 未設定なら空配列で扱いやすく、配列も各項目も元記事から独立させた安心設計〜✨ 設定済み・未設定・ディープコピーを純粋関数テスト3件で固定しましたわ〜🛡️

- **高度検索で記事メタデータまでピンポイントに探せるようになったよ〜!🧩🔎** — `metadata:dc:source`や`metadata:株式会社`みたいに入力すると、RSS・Atom・JSON Feedから保持したカスタム項目のキーと値だけを狙って検索できちゃうっ🎀 大文字小文字を気にせず、フレーズ検索や`-metadata:広告`の除外もいつもの構文でOK〜✨ 構文解析・キー/値・フィールド分離・未設定・フレーズ/否定を回帰テスト4件で固定しましたわ〜🛡️

- **linkなしRSSでもpermalink GUIDから記事を開けるようになったよ〜!🧭🔗** — RSS 2.0の記事に`<link>`がなくても、`isPermaLink="true"`のGUIDを記事URLとして賢く採用っ🎀 属性省略も仕様どおりpermalink扱いにしつつ、`false`の識別子はURLにせず、明示linkの優先と危険スキーム除外もしっかりキープ〜🛡️ 明示true・省略・false・優先順位・安全性を回帰テスト4件で固定しましたわ〜✨

- **Atomの記事リンクが添付ファイルに迷子にならなくなったよ〜!🔗✨** — `enclosure` や `related` が先に並ぶフィードでも、標準の `rel="alternate"` を記事・サイトURLとしてしっかり優先っ🎀 `rel` 省略もAtom仕様どおりalternate扱いにしつつ、非標準フィードは従来のnon-selfリンクへ戻る互換設計だよ〜🛡️ feed・entry・省略形・フォールバックを回帰テスト3件で固定しましたわ〜💫

- **MIMEパラメータ付きのフィードリンクも自動発見できるようになったよ〜!📡🔍** — サイト側が `application/rss+xml; charset=utf-8` みたいに文字コード情報を添えていても、RSS・Atom・JSON Feedのリンクをちゃんと見つけられるようになったよっ🎀 `type` と `href` の属性順はどっちでもOKで、MIME本体に余計な文字をくっつけた不正値はスルーする安心設計〜✨ 実装本体の純粋関数を直接使う4ケースで、3形式・2属性順・防御入力を固定しましたわ〜🛡️

- **Markdown保存に記事カテゴリも付いてくるようになったよ〜!🏷️📝** — 記事をMarkdownで保存・コピーしたりObsidianへ送ったりすると、frontmatterへカテゴリ一覧がYAML配列で入るようになったよっ🎀 元の並び順を保つから、外部ノートアプリでタグ付けや分類を引き継ぐのも超スムーズ〜✨ コロン・引用符・改行は既存の安全なYAMLエスケープで整え、カテゴリなしなら項目自体を出さないスマート設計を純粋関数テスト 3 件で固定しましたわ〜🛡️

- **記事JSONバックアップにカテゴリも一緒に入るようになったよ〜!🏷️💾** — ブックマーク・後で読む・コレクションをJSON保存すると、記事ごとのカテゴリ一覧も表示順そのままで持ち出せちゃうっ🎀 カテゴリ未設定の記事は空配列になるから、外部ツールで分類を復元するときも扱いやすくて超便利〜✨ 元記事とは独立した配列として書き出す安心設計を、設定済み・未設定・配列分離の純粋関数テスト 3 件で固定しましたわ〜🛡️

- **高度検索でRSS要約だけを狙って探せるようになったよ〜!📝🔎** — 検索欄で `summary:モナド` と入力すると、記事タイトルや全文じゃなくフィードの要約欄だけをピンポイント検索できちゃうっ🎀 `summary:"注目ニュース"` のフレーズ検索や `-summary:広告` の除外も既存構文のまま使えて、一覧の短い紹介文から探したいときに超便利〜✨ 構文解析・対象限定・複合構文・空要約を純粋関数テスト 4 件で固定しましたわ〜🛡️

- **高度検索で記事の言語をピンポイントに絞れるようになったよ〜!🌍🔎** — 検索欄で `language:ja` や `language:en-US` と入力すると、JSON Feed の `language` と RSS 拡張の `dc:language` だけを対象に記事を探せちゃうっ🎀 言語以外のメタデータは混ぜず、未設定記事も安全にスルーするから多言語フィードの整理が超スムーズ〜✨ 構文解析・名前空間付きキー・大文字小文字・未設定を純粋関数テスト 4 件で固定しましたわ〜🛡️

- **`application/json` 配信の JSON Feed も直リンクで購読できるようになったよ〜!📡✨** — 推奨 MIME の `application/feed+json` だけじゃなく、公式仕様で許容されている一般的な JSON 配信も自動発見っ🎀 本文をちゃんと JSON として解析し、正規の version ホストと items 配列まで確認するから、普通の JSON API や偽装 URL をフィード扱いしない安心設計だよ〜🛡️ charset・通常 JSON・偽ホスト・壊れた JSON・別 MIME の純粋関数テスト 5 件で固定しましたわ〜💫

- **過去 1 年で読書した日数がひと目で分かるようになったよ〜!📅📚** — 読書統計の年間ヒートマップ下に「読書日数 N日」を追加っ🎀 草グラフを数えなくても、1件以上読んだ日が年間でどれくらいあるか即チェックできて、読書習慣の振り返りがもっと楽しくなっちゃう〜💫 全体統計とフィード別表示の両方に追従し、空データ・0件混在・入力順を純粋関数テスト 3 件で固定しましたわ〜🛡️

- **JSON Feed の記事言語をフィルターに活かせるようになったよ〜!🌍✨** — JSON Feed 1.1 の `language` を記事メタデータへ取り込み、記事ごとの指定を優先しつつ未指定ならフィード全体の言語を賢く継承っ🎀 除外メニューでも「言語」として選べるから、多言語フィードから読みたい言語だけ残す整理が超スムーズ〜💫 空値や不正型は安全にスルーし、優先・継承・防御入力を回帰テスト 4 件で固定しましたわ〜🛡️

- **JSON Feed の画像 attachment もサムネイルに使えるようになったよ〜!🖼️✨** — `image` や `banner_image` がない記事でも、`attachments` に入った最初の `image/*` を自動で拾って一覧のサムネイルに表示っ🎀 音声などの非画像 attachment はスルーし、明示画像の優先順と危険な URL の除外もそのまま守る安心設計だよ〜🛡️ 優先順位・非画像・URL 安全化を含む回帰テスト 4 件で固定しましたわ〜💫

- **コレクションを名前の降順でも並べ替えられるようになったよ〜!🗂️✨** — サイドバーの並び順ボタンに「名前 z-a」を追加っ🎀 これまでの手動・作成日・記事数・名前 a-z に続けて切り替えられるから、末尾側の名前から探したいときもスクロールが超スムーズ〜💫 選んだ順序は既存のブラウザ保存にそのまま対応し、逆辞書順と5軸サイクルを純粋関数テスト 2 件で固定しましたわ〜🛡️

- **高度検索で記事 URL をピンポイントに探せるようになったよ〜!🔎🌐** — 検索欄で `url:example.com` と入力すると、タイトルや本文じゃなく記事 URL だけを対象にドメイン・パス・記事 ID を絞り込めちゃうっ🎀 `url:"posts/123"` のフレーズ検索や `-url:tracking.example` の除外、AND / OR との組み合わせも既存構文のまま使えて超便利〜✨ 空 URL も安全に扱い、パース・フィールド限定・否定・フレーズを純粋関数テスト 4 件で固定しましたわ〜🛡️

- **購読フィードのサイト URL も操作メニューから即コピーできるようになったよ〜!🌐✨** — RSS の配信 URL だけじゃなく、元サイトの URL も「サイト URL をコピー」からワンタップで取得できちゃうっ🎀 サイト URL がないフィードでは操作を出さず、Clipboard API が使えない環境やコピー失敗も既存どおり Toast と開発ログでしっかりお知らせする安心設計だよ〜🛡️

- **過去 1 年でいちばん読んだ「自己ベスト日」が見えるようになったよ〜!🏆📚** — 読書統計の年間ヒートマップ下に、1日あたりの最多読了日と件数を追加っ🎀 草グラフを一つずつ探さなくても「2026年8月5日・12件」みたいにベスト記録がすぐ分かって、読書モチベもアゲアゲ〜💫 同じ件数の日が複数あれば直近日を選び、読書0件なら表示しないスマート設計っ✨ 全体統計とフィード別表示の両方に追従し、入力順・空データ・同数条件を純粋関数テスト4件で固定しましたわ〜🛡️

- **テーマプリセットを JSON でバックアップできるようになったよ〜!🎨💾** — 表示設定のプリセット欄に「JSON保存」を追加っ🎀 お気に入りのテーマ・文字サイズ・フォント・行間・本文幅の組み合わせを、表示順や作成日時ごとそのまま持ち出せるようになりましたわ〜💫 エクスポート日時と件数入りの読みやすいJSONで、ブラウザデータ消去前や別端末へ設定を移すときの参照にも超便利っ✨ 0件なら操作できない安全設計と日付付きファイル名を純粋関数テスト3件で固定しましたわ〜🛡️

- **記事本文を `.txt` でサクッと保存できるようになったよ〜!📄✨** — 共有メニューに「テキストをダウンロード」を追加っ🎀 全文取得済みならその本文、まだなら RSS 要約を使って、見出し・段落・リストの改行を読みやすく整えたプレーンテキストとして保存できちゃう〜💫 ファイル名は記事タイトルから安全に生成し、空タイトルでも `article.txt` にフォールバックする安心設計を純粋関数テスト 2 件で固定しましたわ〜🛡️

- **過去 1 年でいちばん続いた読書日数が見えるようになったよ〜!🔥📚** — 読書統計のサマリーに「1年最長」カードを追加っ🎀 今の連続記録だけじゃなく、年間ヒートマップから過去ベストの読書ストリークを自動で見つけるから、「あの頃めっちゃ読んでた!」がひと目で振り返れちゃう〜💫 月末・年末をまたぐ連続日も入力順に関係なく正しく数え、読書ゼロなら 0 日になる安心設計を純粋関数テスト 4 件で固定しましたわ〜🛡️

- **保存済み検索条件を JSON でバックアップできるようになったよ〜!🔎✨** — 設定の「インポート・エクスポート」から、名前を付けて保存した高度な検索式をワンタップで持ち出せるようにしたよっ🎀 表示順・名前・検索式・作成日時をそのまま保って、エクスポート日時と件数も入った読みやすい JSON だから、別端末で検索式を再登録するときの参照やブラウザデータ消去前のバックアップに超便利〜💫 0 件なら誤操作できない安全設計で、ファイル名と整形済み本文も純粋関数テスト 3 件で固定しましたわ〜🛡️

- **過去 1 年の読書履歴を CSV で持ち出せるようになったよ〜!📊✨** — 読書統計モーダルに「過去 1 年を CSV で保存」を追加っ🎀 日付と読了数を 365 日分そのまま出力するから、Excel やスプレッドシートで自分の読書ペースを分析したりバックアップしたりできちゃうよ〜💫 日本語ヘッダーが文字化けしにくい UTF-8 BOM + CRLF 形式で、ゼロ件の日も空データも安全に扱える設計を純粋関数テスト 3 件で固定しましたわ〜🛡️

- **記事本文をプレーンテキストでワンタップコピーできるようになったよ〜!📋✨** — 共有メニューに「本文をテキストコピー」を追加っ🎀 HTML の見出し・段落・リストを読みやすい改行付きテキストへ整えて、チャットやメール、文書へ装飾なしでそのまま貼れるよ〜💫 全文取得済み本文を優先し、未取得時は RSS 要約へフォールバック、空データも安全に扱う設計を純粋関数テスト 3 件で固定しましたわ〜🛡️

- **記事をメールでサクッと共有できるようになったよ〜!💌✨** — 共有メニューとクイック共有の選択肢に「メールで共有」を追加っ🎀 既定のメールアプリを開いて、件名には記事タイトル、本文にはタイトルと URL をセットするから、そのまま宛先を選んで送れちゃうよ〜💫 日本語・記号・クエリ付き URL も安全にエンコードし、URL 生成と保存済み設定の復元を純粋関数テスト 2 件で固定しましたわ〜🛡️

- **読んでる記事をそのまま Markdown ファイルで保存できるようになったよ〜!📝✨** — 共有メニューに「Markdown をダウンロード」を追加っ🎀 frontmatter + 本文入りの `.md` をワンタップでローカル保存できるから、好きなエディタや Git 管理へ持っていくのも超スムーズ〜💫 ファイル名は記事タイトルから安全に生成し、使えない文字は既存サニタイズで整形、空タイトルでも `article.md` にフォールバックする安心設計だよ〜🛡️ 純粋関数テスト 2 件で内容とファイル名を固定しましたわ〜🌸

- **購読中フィードの URL を操作メニューから即コピーできるようになったよ〜!📡✨** — フィードの共有・動作確認・別リーダーへの登録で使いたいとき、詳細画面から長い URL を手選択しなくても「フィード URL をコピー」を押すだけっ🎀 成功は Toast でお知らせ、Clipboard API が使えない環境やコピー失敗もエラー表示 + 開発ログでちゃんと気づける安心設計だよ〜🛡️ action の順序とハンドラ配線も回帰テストで固定しましたわ〜💫

- **記事の出典付き Markdown をワンタップでコピーできるようになったよ〜!📚✨** — 共有メニューに `[記事タイトル](URL) — 著者 · フィード名 · 公開日` 形式のコピーを追加っ🎀 ノートや文書へ貼るだけで出典が伝わって超便利、情報がない項目や不正な日付は賢く省略するからどんなフィードでも崩れないよ〜🛡️ Markdown 特殊文字のエスケープも既存リンクコピーと共通化し、純粋関数テスト 4 件でばっちり固定しましたわ〜💫

- **おすすめフィードが「実際に反応した記事」から好みを学ぶようになったよ!✨** — フィード推薦 Phase 2a として、これまでエンゲージメント上位フィードの最新記事を一律に AI へ渡していたトピック抽出を、**いいね・ブックマーク・全文取得・元記事を開く・後で読む**の行動対象記事から優先選択する方式へ強化〜🎀 既存 `engagement-score` のアクション重み × 7 日半減期を記事単位にも再利用し、複数行動の合計と新しさを反映、対象記事が足りない場合だけ最新記事で補完するから興味の中心を保ちつつ多様性も維持するよ〜💫 空タイトル・重複タイトル除外と最大 5 件制限も追加、行動履歴なしなら従来どおり最新記事順で完全互換〜🛡️ Workers AI は既存モデル・1 回呼び出しのままで、API response / R2 schema / UI 変更も追加推論コストもなし! 純粋関数テスト 5 件で優先順・時間減衰・fallback・防御入力・重複除外を固定しましたわ〜🌸

## 2026-08-03

### バグ修正っ

- **重複記事が filtered に混入したときの nextArticle 遷移が同 id 記事に stuck するバグを解消!💡** — ユーザー報告「重複して記事が取得される場合があり、正常に次の記事へ遷移できない」への対応、`useArticleData.fetchAndSetArticles` の initial fetch で dedup が適用されていない drift + `useArticleNavigation.nextArticle` の `filtered[currentIndex + 1]` 素朴 index+1 pattern が同 id 重複時に stuck する既存 bug を 2 層 defensive で修正いたしました〜🎀 **真因**: サーバー側 `/api/articles` response に id / link / guid 重複が混入する経路 (複数 feed で同記事 / shared-feed merge のズレ等) があり、`polling` / `loadMore` は既に `mergeUniqueArticles` 経由で dedup 済だったが、`fetchAndSetArticles` (initial fetch) のみ `setArticles(data)` で素通し → filtered に重複伝播 → `useArticleNavigation.findIndex + index+1` pattern が同 id 記事に遷移して「次の記事に進めない」体感バグを生む〜💫 **修正 Layer 1 (fetch 層)**: `fetchAndSetArticles` の `setArticles(data)` を `setArticles(mergeUniqueArticles([], data))` に変更 (empty base に merge = dedup 適用)、initial fetch でも polling / loadMore と同 dedup 経路に統一〜🛡️ **修正 Layer 2 (navigation 層)**: `useArticleNavigation.nextArticle` / `prevArticle` を `filtered[currentIndex + 1]` 直接参照から **`currentIndex + 1` から線形検索で「最初の異なる id 記事」を返す** ロジックに変更、filtered 内に同 id 記事が万一残っていても navigation 破綻しない defensive layer 追加〜✨ two-layer defense で fetch 層と navigation 層の両方で防御、touch 2 file (`useArticleData.ts` + `useArticleNavigation.ts`) / net +25 line、機能変化ゼロ (重複なしのケースは既存 flow と完全同一)、`prevArticle` / `nextArticle` も両方向 skip logic 対称化〜🌸 追加検討: サーバー側 `/api/articles` endpoint dedup 強化は Layer 3 として **今回 scope 外**、client 側 2 層 defensive で症状は完全解消 (サーバー修正は根本原因除去として別 Issue 判断)〜🎀

### パフォーマンス改善っ

- **useCollections + useFeedGroups にも localStorage cache 復元を横展開 (Phase 4)!⚡** — 前 Phase 3 (useFeedData cache 復元) の canonical pattern を sidebar 常時表示の 2 sibling hook (`useCollections` / `useFeedGroups`) にも横展開いたしました〜🎀 修正前は auth 完了 → 3 endpoint (`/api/feeds` + `/api/collections` + `/api/feed-groups`) の並列 fetch 待ちで sidebar の 3 section (feeds / collections / groups) が empty state、修正後は **全 3 section が localStorage cache から optimistic 復元** で initial paint 直後に前回内容の sidebar 全体が populated 状態で見える〜💫 `STORAGE_KEYS.CACHED_COLLECTIONS` + `CACHED_FEED_GROUPS` 追加、各 hook に minimal type guard (`isCollectionArray` / `isFeedGroupArray`、Phase 3 の `isFeedArray` と同 pattern で必須 field のみ検証)、`useMemo` で mount 時 1 回だけ `loadJsonObject` 復元 → `useAsyncFetch` の `initialData` に渡す + `transform` hook 内で `storageSet` cache 更新 (fetch 成功時のみ、エラー時は cache 維持で offline UX 継続、useAuth CACHED_USER と同挙動)〜🛡️ touch 3 file (`storage.ts` に 2 key 追加 + `useCollections.ts` + `useFeedGroups.ts`) / net +55 line、機能変化ゼロ (initial 表示の perceived load time 短縮のみ)〜✨ Phase 1 (Piper wasm chunk 除外 `33f035db`) + Phase 2 (useRecommendations + usePushNotifications initial fetch defer `8357f6d7`) + Phase 3 (useFeedData cache 復元 `baf8ee50`) + Phase 4 (本 commit) で **初期表示 critical path の 4 段最適化** 完了、sidebar の 3 主要 section が **前回セッションの内容ですぐ populated 表示**〜🌸 残候補 (useReadState は既に localStorage primary source で cache 復元済、useArticleData は articles 数百件で cache size 大 + refactor 中規模 / AppShell 55 hook 分割 / SSR 部分復活 / Cloudflare Workers cold start 対策) は各々 refactor 大規模 or 実測依存で 1 サイクル完結困難、Phase 1-4 実測後の判断予定〜🎀

- **useFeedData に localStorage cache 復元を追加、sidebar の初期表示ちらつきを解消 (Phase 3)!⚡** — 初期表示 10 秒問題対応の Phase 3、`useFeedData` の `useState<Feed[]>([])` empty initializer を **前回セッションの `localStorage` cache から optimistic 復元** する canonical pattern に変更いたしました〜🎀 修正前は auth 完了 → `/api/feeds` fetch 待ち (数百 ms - Cloudflare Workers cold start で数秒) の間 sidebar が empty state で skeleton loading 表示 = **perceived load time が fetch 完了まで遅延**、修正後は **`localStorage` cache から前回 feeds を先出し** → fetch 完了で最新に上書き = ユーザー視点で **initial paint 直後に前回内容の sidebar が populated 状態で見える**〜💫 `STORAGE_KEYS.CACHED_FEEDS` (`"rss-cached-feeds"`) 追加 + `isFeedArray` minimal type guard (`id` / `url` / `title` / `siteUrl` の必須 field 検証、canonical parallels: `useAuth#isUserProfileOrNull` と同 pattern) + `useState` initializer で `loadJsonObject<Feed[]>` 復元 + fetch 成功時 `storageSet` で cache 更新〜🛡️ fetch エラー時 (catch) は cache 維持で offline / 一時的 network 障害でも前回 feeds 表示継続 (`useAuth` `CACHED_USER` と同挙動)、stale cache の Feed field 増加ケースでも minimal type guard で crash 防止 + fetch 完了で正しい state に上書き〜✨ touch 2 file (`storage.ts` に `CACHED_FEEDS` key 追加 + `useFeedData.ts` に type guard + initializer + storageSet) / net +30 line、機能変化ゼロ (initial 表示の perceived load time 短縮のみ、fetch flow は既存 flow と完全同一)、Phase 1 (Piper wasm chunk 除外) + Phase 2 (useRecommendations + usePushNotifications initial fetch defer) と併せて **初期表示 critical path の 3 段最適化** 完了いたしましたわ〜🌸 追加改善候補 (useReadState / useCollections / useFeedGroups の cache 復元 / AppShell 55 hook 分割 / SSR 部分復活) は各々 refactor 中規模で 1 サイクル完結困難、Phase 1-3 実測後の判断予定〜🎀

- **useRecommendations + usePushNotifications の initial fetch を `setTimeout(0)` で initial paint 後に defer!⚡** — 初期表示 10 秒問題対応の Phase 2、`useRecommendations` (auth 完了直後の `useAsyncFetch({ auto: true })` で `/api/recommendations` fetch 発火) + `usePushNotifications` (auth 完了後の `navigator.serviceWorker.ready` 待ち + `getSubscription` check) の 2 hook を **initial paint 後の task queue で execute** するように defer 化いたしました〜🎀 各 hook に `deferReady` state + `setTimeout(0)` trigger を追加、既存 fetch trigger 条件に `user && deferReady` を組み合わせて **initial render が完了するまで network / IO を発火させない** 設計〜💫 修正前は auth 完了 → useFeeds + useReadState + useRecommendations + usePushNotifications + useCollections + useFeedGroups 等が **一斉に並列 fetch** で初期表示 critical path に competing network cost、修正後は critical path に必要な hook (useFeeds / useReadState / useCollections / useFeedGroups) のみが initial render 段階で発火、推薦 + Push 通知 check は initial paint 後の idle 時に execute〜🛡️ trade-off: 推薦 section が最初の 1 tick 空リスト表示 (default `initialData: []` で保証済、UX 影響ゼロ)、設定モーダル open 時の subscribed state 初期 render で false (setTimeout(0) 完了までの数 ms、モーダル open は user action で数百 ms 以上経過しており不可視)〜✨ touch 2 file (useRecommendations.ts + usePushNotifications.ts) / net +30 line、機能変化ゼロ (defer 完了後は既存 flow と完全同一、user 変化時の setDeferReady(false) reset も追加)、Phase 1 (Piper wasm chunk 除外 `33f035db`) と併せて **初期表示 critical path から Web Speech default user 向けの network / IO を最大削減**〜🌸 追加改善候補 (AppShell 55 hook 遅延化 / Suspense boundary 追加 / SSR 部分復活 / Cloudflare Workers cold start 対策) は各々 touch 数十 file 級の大規模 refactor で 1 サイクル完結困難、Phase 1 + Phase 2 実測後の判断予定〜🎀
...
- **AiNotificationTabPanel の TIMEZONE `<option>` 440 React 要素を module-level に hoist (chain 5 完成)!⚡** — auditor-perf agent 11 回目派遣 (chain 5 の 3 番目、confidence 88%) が発見した前 perf 10th 積み残しの canonical extension を修正〜🎀 前 perf 10th (`c61888f2`) で `TIMEZONES` 配列自体は module-level `Object.freeze` 集約済だったが、実際の `<option>` 要素生成 `{TIMEZONES.map((tz) => <option key={tz} value={tz}>{tz}</option>)}` は component body 内に残存で、1 render あたり **~440 React 要素 + prop object を allocate** する drift 状態を解消〜💫 UserSettingsModal は `useReaderSettings` Context を subscribe しており、`useReaderSettingsValue` (47 field useMemo) 内の 1 値変化で 4 tab panel 全てが re-render、`galleryMinImagePx` `<input type="range">` slider drag の秒間数十回 onChange 発火で **hidden tab (`hidden={activeTab !== "ai-notifications"}`) 内 440 element 再生成 + reconciliation コスト** が累積していた perf 罠を、`TIMEZONE_OPTIONS` module-level 定数への hoist (`const TIMEZONE_OPTIONS = TIMEZONES.map((tz) => <option key={tz} value={tz}>{tz}</option>)`) + JSX 展開 `{TIMEZONE_OPTIONS}` に置換で解消〜🛡️ React elements は immutable props object のため module 間再利用が safe、canonical (前 perf 10th の TIMEZONES module hoist の延長)、touch 1 file / net +7 line (module-level 定数 + JSDoc 追加)、機能変化ゼロ (visual + behavior 完全同一、slider drag 時の hidden tab reconciliation waste 完全解消)〜✨ 前 perf 10th commit body のコメント「`<option>` 子要素の reconciliation は `key={tz}` 駆動なのでこの hoist では変わらない」で明示的に積み残しと予告した delayed fix、chain 12 recovery pattern と併せて **agent rotation strategy の self-consistent perf sweep** を実証〜🎀 **chain 5 完成 (a11y 10th + simplify 10th + perf 11th の 3 観点 canonical validation cycle 到達)、5 chain × 3 観点 = 15 audit slot 完成**、前 4 chain 全 3 観点完成 + chain 11 pattern break + chain 12 recovery の anti-fragile + self-healing 実証と併せて agent rotation strategy を extended validation いたしましたわ〜🌸

### リファクタリングっ

- **user-settings 9 sites の SegmentGroup options 同形 pattern を `buildLabeledOptions` helper に集約!🧹** — auditor-simplify agent 10 回目派遣 (chain 5 継続、confidence 92%) が発見した helper drift を修正〜🎀 FontSection.tsx (2 sites: FONT_SIZE_CYCLE / FONT_FAMILY_CYCLE) + LayoutSection.tsx (2 sites: LINE_HEIGHT_CYCLE / CONTENT_WIDTH_CYCLE) + GallerySection.tsx (5 sites: GALLERY_COLUMNS_CYCLE × 2 with LABELS/FOCUS_LABELS / GALLERY_CARD_SIZE_CYCLE / GALLERY_AUTO_SCROLL_SPEEDS / GALLERY_PAGE_SIZE_CYCLE) の **計 9 sites 全て** で `options={<CYCLE>.map((v) => ({ value: v, label: <LABELS>[v] }))}` 同形 4 line inline pattern が重複していた drift を、`shared.tsx` に純粋関数 `buildLabeledOptions<T extends string | number>(cycle: readonly T[], labels: Record<T, string>): SegmentOption<T>[]` 集約 + 全 9 sites を 1 line `options={buildLabeledOptions(<CYCLE>, <LABELS>)}` 呼出に置換〜💫 canonical (`helper-drift.md § 派生ケース「同形 useCallback 8 行ブロックが 4+ で重複」` + `react-component-split.md § 派生ケース「同形 JSX ラッパーが 3 回以上重複」`) の閾値 3+ / 4+ を **9 sites でぶっちぎり超過**、既存 sibling canonical (`TTL_OPTIONS` module-level frozen) が示す「SegmentGroup options は static / helper 集約」design intent と integrity〜🛡️ `SegmentOption<T>` interface も module-private から `export` 化 (将来別 SegmentGroup 呼出時に type 参照可能)、`AutoReadSection` の `${v}%` dynamic label (template literal で LABELS Record 不使用) は semantic gap で対象外〜✨ touch 4 file (shared.tsx + FontSection + LayoutSection + GallerySection) / net -18 line、機能変化ゼロ (visual + behavior 完全同一、per-render `.map` allocation は helper 内で維持)、chain 5 の 2 番目 (simplify 10th) 完成いたしましたわ〜🌸 将来 SegmentOption field 追加時の 9 sites 同期修正義務が 1 箇所に閉じ込められ、drift 累積を構造的予防〜🎀
...
- **TIMEZONES + TTL_OPTIONS に `Object.freeze` 追加 + rationale nit 修正 (chain 11 adversarial verify で発見)!🔒** — code-reviewer agent 14 回目派遣 (前 commit `c61888f2` AiNotificationTabPanel TIMEZONES hoist の adversarial verification) が前 10 chain 全 pass を破って発見した 1 CONFIRMED finding (R3、confidence 85%) を修正〜🎀 **問題**: 前 commit の TIMEZONES は `readonly string[]` type-annotation only (compile-time guarantee) で `Object.freeze` 未適用、canonical parallels (`empty-sentinels.ts` の `EMPTY_STRING_SET` / `EMPTY_STRING_ARRAY` / `EMPTY_FEED_ARRAY` 3 sentinel + JSDoc「`Object.freeze` で `.add()` / `.push()` による sentinel 汚染を runtime safety net で防御」) と runtime protection level が乖離、consumer が `as string[]` で cast away readonly + mutate 経路で silent module singleton corruption リスクありの状態〜💫 **修正**: (1) TIMEZONES に `Object.freeze(...)` wrap で primary canonical と完全一致、(2) sibling drift precedent の `TTL_OPTIONS` (shared.tsx L21、canonical parallels JSDoc 明記の pattern から drift 状態) にも `Object.freeze([...]) as {...}[]` cast で **sibling drift 一括横展開 canonical** 適用、canonical parallels JSDoc 「consumer 側の型変更を要求しない」pattern に完全準拠 (SegmentGroup `readonly SegmentOption<T>[]` prop 互換 verify 済)〜🛡️ 加えて前 commit RELEASE_NOTES.md 記述の **R4 rationale nit** (「440 `<option>` 子要素の new-identity reconciliation」claim は technically inflated、React `<option>` reconciliation は `key={tz}` string 駆動で parent array identity 依存でない) を明示訂正、真の perf win は **ICU-call skip (~0.5-2ms) + `.map` closure allocation skip** で reconciliation cost は不変〜✨ touch 2 file (`AiNotificationTabPanel.tsx` + `shared.tsx`) / net +2 line (Object.freeze wrap + JSDoc 追加) の sibling drift 一括横展開、機能変化ゼロ (visual + behavior 完全同一、runtime immutability 追加のみ)、`react-state-ref.md § 派生「モジュールレベル sentinel オブジェクトは Object.freeze で下流汚染を防ぐ」` primary canonical への完全準拠状態に復帰〜🌸 **chain 11 意義**: 前 10 chain 全 pass の「find → adversarial verify all-pass」pattern を破って agent が genuine canonical-adherence gap を独立発見、`feedback_subagent_verification.md § adversarial verify 価値`「10 chain 連続 all-pass 後の chain 11 で真の finding を検出する adversarial verify の precision」を実証、agent rotation strategy の canonical validation として anti-fragile 特性を証明いたしましたわ〜🎀

- **AiNotificationTabPanel の `Intl.supportedValuesOf("timeZone")` を module-level constant に集約 (通知タブ open 中の re-render churn 解消)!⚡** — auditor-perf agent 10 回目派遣 (chain 4 完成: 未 audit modal 群 5+ 件 = FeedHealthModal / ReadingStatsModal / ImageDownloadModal / AppModals / reading-stats sub / user-settings sub を perf 観点で新規 audit、confidence 90%) が発見した module-level sentinel drift を修正〜🎀 AiNotificationTabPanel.tsx:66-67 で `const timezones = typeof Intl.supportedValuesOf === "function" ? Intl.supportedValuesOf("timeZone") : []` を component body で毎 render 呼び出していた drift、Chromium が 440 件の timezone 配列を fresh identity で毎回返す → 通知タブ open 中の silent hours `<input type="time">` keystroke / timezone `<select>` change / errorNotificationsEnabled toggle / pushConfigLoading 遷移 / 3 debounce chain (silentStart / silentEnd / timezone 1000ms) の各 re-render 起点で ICU list 再構築 (~0.5-2ms) + 440 `<option>` 子要素の new-identity reconciliation が走る perf コストを解消〜💫 module-level `const TIMEZONES: readonly string[] = ...` に hoist、component body 内 3 参照 (declaration + `TIMEZONES.length` 条件 render + `TIMEZONES.map`) を canonical 統一〜🛡️ `react-state-ref.md § 派生ケース「モジュールレベル sentinel オブジェクトは Object.freeze で下流汚染を防ぐ」` 規範の module-level sentinel pattern 適用、canonical parallels (`EMPTY_SET` / `EMPTY_STR_ARRAY` / `EMPTY_FEED_ARRAY` の 3 sentinel + `TTL_OPTIONS`) と integrity、touch 1 file / net ~5 line 変更、機能変化ゼロ (visual + behavior 完全同一、timezone list は per-session invariant で initial evaluation 結果一致)〜✨ 他 audit target (FeedHealthModal / ReadingStatsModal / ImageDownloadModal / AppModals / reading-stats sub-components / user-settings 他 sub-components) は canonical 完全準拠 audit clean 判定確認 (dynamic ssr:false code-splitting / useMemo deps 正確 / useInboxProgress の useFeedStructuralSignature / FeedHealthModal untilLabel の frozen-state 引数化 pattern と整合)、**chain 4 完成 (a11y 9th + simplify 9th + perf 10th の 3 観点 canonical validation cycle 到達)** いたしましたわ〜🌸
...
- **FeedHealthModal の 3 section shell 重複を file-local helper (FeedHealthSection) に集約!🧹** — auditor-simplify agent 9 回目派遣 (前 a11y 9th と同 未 audit modal 5 群を simplify 観点で新規 audit、confidence 85%) が発見した 3-site 同 file duplication を修正〜🎀 FeedHealthModal.tsx で errors (L111) / rate-limited (L168) / oversize (L219) の 3 section shell (`<section className="px-4 pt-4 pb-2" aria-labelledby="fhm-section-...">` + `<h3 id className="text-[10px] font-medium tracking-[0.25em] uppercase text-text-muted mb-2">TITLE</h3>`) が byte-identical (id + title text のみ differ) の pattern を、file-local `FeedHealthSection` helper (~30 lines) に集約〜💫 `react-component-split.md § 派生ケース「同形 JSX ラッパーが 3 回以上重複 → ポリモーフィック as props 付きラッパーコンポーネント化」` canonical 適用、前 simplify 5th `MenuSectionHeader` (FeedContextMenu 4 site) + 前 simplify 8th `LightboxRoundButton` (ImageLightbox 3 site) 実績と同 pattern〜🛡️ Props: `id` + `title` + `children`、3 call sites を 1 line 呼出しに collapse、`fhm-section-<name>` id prefix drift risk 解消 (新 section 追加時に id-prefix pattern を思い出す必要なし)〜✨ `<h3>` typography は `design-system.md § タイポグラフィ` canonical セクションヘッダー、前 a11y 9th (`c1c3ef20`) で ReadingStatsModal に周知済 canonical mirror pattern と integrate、機能変化ゼロ (visual + behavior 完全同一、rate-limited section の rich title は string 単純化で verify)、touch 1 file / net ~9 line reduction いたしましたわ〜🌸 Skipped candidates: FeedFilterModal TagInput (2 site 未達) / FeedAddModal 2 collapsible sections (2 site 未達) / FeedHealthModal 4 summary dot indicators (trivial 2-line pattern、helper で ~5 line 増、compelling 弱い) は `code-simplifier` skill 「premature abstraction 防止」canonical で見送り〜✨

- **ReadingStatsModal 4 section header の `<span>` を `<h3>` に修正 (WCAG 1.3.1 準拠、canonical sibling FeedHealthModal と統一)!⌨️** — auditor-a11y agent 9 回目派遣 (未 audit modal 5 群を新規 target 化、confidence 92%) が発見した canonical drift を修正〜🎀 ReadingStatsModal.tsx の 4 section header (「過去 1 年」L137 / 「直近 7 日」L146 / 「よく読むフィード」L174 / 「フィード別 未読消化率」L216) が `<span className="text-[10px] font-medium tracking-[0.25em] uppercase text-text-muted">` で render されていた drift を、canonical sibling `FeedHealthModal.tsx:113-117` の `<h3 id={...}>` pattern と同型化〜💫 `design-system.md § タイポグラフィ` で当該 className を「セクションヘッダー」canonical typography 明記済 → semantic tag は `<h3>` が canonical、tag 変更のみで className 完全同一 (visual 変化ゼロ)〜🛡️ 修正前は screen reader (NVDA / JAWS / VoiceOver) の rotor / H キー見出しナビゲーションが ReadingStatsModal 内で機能せず、統計モーダル内の視覚的セクション構造が AT ユーザーに programmatically 認識不能な WCAG 1.3.1 (Info and Relationships) 違反状態を解消、長い modal 内で目的セクションに direct navigation 可能に〜✨ 他 4 modal (FeedAddModal / FeedFilterModal / FeedHealthModal / FeedQuickSwitchModal) は全 canonical 準拠 audit clean 判定 (Modal.tsx wrap + combobox 5 属性 + Disclosure 3 点セット + 44px touch target + 装飾 SVG aria-hidden + `<h3 aria-labelledby>` landmark)、touch 1 file / 4 line 変更で完結いたしましたわ〜🌸

- **ImageLightbox の 3 round-button chrome 重複を file-local helper (LightboxRoundButton) に集約!🧹** — auditor-simplify agent 8 回目派遣 (前 a11y 8th と同 5 sub-components を simplify 観点で新規 audit、confidence 85%) が発見した 3-site 同 file duplication を修正〜🎀 ImageLightbox.tsx で close (L89-106) / prev (L109-128) / next (L131-150) の 3 button が共通 chrome (`w-11 h-11 rounded-full bg-black/40 text-white hover:bg-black/60 flex items-center justify-center transition-colors` + `disabled:*` variants + SVG defaults) を持つ pattern を、file-local `LightboxRoundButton` helper (~35 lines) に集約〜💫 `react-component-split.md § 派生ケース「同形 JSX ラッパーが 3 回以上重複 → ポリモーフィック as props 付きラッパーコンポーネント化」` canonical 適用、前 simplify 5th の `MenuSectionHeader` / `MenuRadioOption` (FeedContextMenu 7 site) 実績と同 pattern〜🛡️ Props: `positionClass` + `disabled?` + `ariaLabel` + `onClick` + `children` (SVG)、`disabled:*` prefix は Tailwind の disabled attr 有無で自動 gate、close button (disabled 非使用) にも harmless 適用で 3 site で完全共通 className を実現〜✨ backdrop-specific styling (`bg-black/40`) のため sibling module に export せず co-located 維持 (agent grep で `bg-black/40.*rounded-full` は本 file のみ確認)、機能変化ゼロ (visual + behavior 完全同一)、touch 1 file / net ~10 line reduction いたしましたわ〜🌸 Piper 2 toasts chrome drift (2 site) は canonical 閾値 (3+) 未達で `code-simplifier` skill 「premature abstraction 防止」canonical で見送り、次サイクル 3 site 到達時対応候補として記録〜✨

- **NSFWEyeAnimation に status announcement + 装飾 SVG aria-hidden 追加 (WCAG 4.1.3 + 1.3.1 準拠)!⌨️** — auditor-a11y agent 8 回目派遣 (未 audit `src/components/` sub-components 5 files: ImageLightbox + Piper 系 3 + NSFWEyeAnimation を新規 target 化、confidence 85%) が発見した status message + 装飾 SVG a11y drift を修正〜🎀 NSFWEyeAnimation.tsx (NSFW mode 起動全画面演出 2.8s) の外側 `<div>` に **`role="status" aria-live="polite"`** 追加 + SVG root に `aria-hidden="true"` 追加、canonical (sibling batch 内 `PiperInitProgressToast.tsx:29-30` 同 pattern) と mirror〜💫 修正前は screen-reader ユーザーが NSFW mode 起動を programmatic に認識できない (WCAG 4.1.3 Status Messages 違反) + 装飾 SVG shapes (12 iris texture lines + 9+5 lashes + iris/pupil/highlight circles) が AT で surface される可能性 (WCAG 1.3.1 Info and Relationships 違反) 状態を解消〜🛡️ visible text "nsfw mode" (L149-167) は AT expose 継続、JSDoc L11-19 の「テーマ非依存の装飾イラスト」意図を AT tree に propagate、機能変化ゼロ (visual 変化なし、semantic ARIA 属性追加のみで screen reader announcement 改善)〜✨ 他 4 files (ImageLightbox / PiperInitProgressToast / PiperErrorDetailToast / PiperEngineHost) は全 canonical 準拠 audit clean 判定確認 (`useModalFocusTrap` + `<h2 id={titleId}>` sr-only + `role="alert"` + touch target 44px 二重指定 + Rules of Hooks 遵守)〜🌸
