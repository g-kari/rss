...
- **\`parseKeywordFilter\` を \`keyword-filter.ts\` に集約** — \`app/api/read-state/route.ts\` の \`extractGlobalFilter\` と \`app/api/feeds/[id]/route.ts\` のインラインフィルターパースを共通関数 \`parseKeywordFilter\` に統一。重複していたパースロジックを1箇所に集約し、両ルートのコードを削減。
