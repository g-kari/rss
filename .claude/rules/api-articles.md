---
paths: "app/api/articles/**,app/api/read-state/**,app/api/content/**,app/api/clip/**"
...
| `q`        | string | サーバーサイド全文検索クエリ (#908 案 A)。全購読フィードの全ページ (latest + p2..pageCount) + 手動保存記事を R2 走査し、`full-text-search.ts` のクエリ構文 (`title:` / `author:` / `feed:` / `category:` / `content:` / `tag:` / `OR` / `-除外` 等) で評価したマッチ記事を `compareByDateDesc` 降順・最大 `MAX_USER_ARTICLES` 件で返す。空 / パース不能クエリは `[]`。全ページ走査のため latency 大 (Cloudflare Cache 不使用、`q` 指定時は他パラメータより優先)。 |
