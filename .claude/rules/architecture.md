...
src/
  ...
  lib/
    ...
    cache-helper.ts          # Cloudflare Cache API 共通ヘルパー（buildCacheKey / matchCfCache / cachePut / cachePutAsync / deleteCfCache / purge*Cache）— 内部 getCfCache() が `caches` 未定義環境 (dev) を判定して全 helper を cache bypass にフォールバックさせる
...
- **Phase 2 (完了)**: `src/lib/` (143 files) 機能別グループ化の subsection (下記参照)
...
`src/lib/` には 143 ファイル の純粋関数 (`release-notes-data.ts` は自動生成 / gitignored のため除外) / ヘルパー / ラッパーが配置される。Workers + ブラウザ両環境で再利用される基盤レイヤーとして責務境界を明確にするため、以下の **機能別グループ + 採用すべき canonical pattern** に従う。

### グループ分類

| グループ                    | 主な責務                                                                              | 代表ファイル                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| --------------------------- | ------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **HTML 処理 pipeline**      | 本文抽出・ノイズ除去・画像/動画/embed 変換・sanitize                                  | `content.ts` / `html-post-processor.ts` / `html-noise-removal.ts` / `html-image-processors.ts` / `html-video-processors.ts` / `html.ts` / `readability-extractor.ts` / `regex-extractor.ts` / `html-embed-transforms.ts` / `html-media-processors.ts` / `html-srcset.ts`                                                                                                                                                                                                                                                                             |
...
| **記事 / フィード処理**     | RSS パース・記事フィルタ・既読マージ・TTL・signature                                  | `xml-parser.ts` / `article-filter.ts` / `article-utils.ts` / `article-ttl.ts` / `read-state-merge.ts` / `read-state-prune.ts` / `feed-discovery.ts` / `feed-signature.ts` / `feed-groups.ts` / `article-filter-equality.ts`                                                                                                                                                                                                                                                                                                                          |
| **AI / TTS**                | Workers AI / browser AI / TTS adapter / voice 選択 / sentence                         | `ai-cache.ts` / `ai-route-helper.ts` / `ai-models.ts` / `browser-summarizer.ts` / `browser-translator.ts` / `tts-adapter.ts` / `tts-text.ts` / `tts-voice.ts` / `tts-sentences.ts` / `tts-dom.ts` / `ai-summary-parse.ts` / `browser-ai-common.ts` / `tts-volume.ts` / `tts-scroll.ts`                                                                                                                                                                                                                                                               |
...
| **ギャラリー / レイアウト** | masonry 計算・autoscroll・loadmore cooldown・scroll direction                         | `gallery-display.ts` / `gallery-prefetch.ts` / `gallery-explode.ts` / `gallery-autoscroll.ts` / `gallery-masonry-layout.ts` / `loadmore-cooldown.ts` / `scroll-direction.ts` / `bulk-selection.ts`                                                                                                                                                                                                                                                                                                                                                   |
| **自動モード / 自動既読**   | autoMode 状態遷移・persist・debug gate                                                | `auto-read.ts` / `auto-read-persist.ts` / `auto-read-debug.ts` / `auto-ai-fallback.ts` / `bgaudio-debug.ts` / `piper-debug.ts` / `debug-helper.ts`                                                                                                                                                                                                                                                                                                                                                                                                   |
...
| **プラットフォーム / 基盤** | storage / dev-log / serialize / concurrency / type guards / 位置計算 / empty sentinel | `storage.ts` / `dev-log.ts` / `serialize-error.ts` / `serialize-async.ts` / `concurrency.ts` / `type-guards.ts` / `modal-focus.ts` / `menu-class.ts` / `context-menu-position.ts` / `selection-popup-position.ts` / `popup-lock.ts` / `empty-sentinels.ts` / `log-sanitize.ts` / `article-ui-helpers.ts` / `article-view-fab.ts` / `reading-stats-level.ts` / `theme-preset.ts`                                                                                                                                                                      |
...
- **`.test.ts` vs `.spec.ts`**: vitest unit test は `.test.ts` (現状 `src/lib/*.test.ts` 例: `article-utils.test.ts` / `binary-proxy-handler.test.ts` 等 16 件、`src/hooks/*.test.ts` / `src/hooks/*.test.tsx` 例: `useArticleListItemProps.test.ts` / `usePiperTts.test.ts` 等 17 件、`src/hooks/__tests__/*.test.ts` / `src/components/*.test.tsx` 例: `FeedHealthModal.test.tsx`)、playwright e2e は `e2e/*.spec.ts`
...
| `cache-helper.test.ts`                          | `src/lib/cache-helper.ts` — `caches` 未定義環境 (dev) のガード spec（`getCfCache()` により全 helper が throw せず cache bypass に倒れることを固定 + 提供環境で `caches.default` に委譲する通常経路も併せて assert、9 ケース網羅）                                                                                                                          |
...
| `log-sanitize.test.ts`                          | `src/lib/log-sanitize.ts#sanitizeLogUrl` — ログ出力用 URL サニタイズ純粋関数（CRLF 除去 / 既定 256 truncate / maxLength 明示 / 除去→truncate の順序、8 ケース網羅、#1257）                                                                                                                                                                                 |
| `type-guards.test.ts`                           | `src/lib/type-guards.ts#isPlainObject` — plain object 3 軸 narrowing 純粋関数（null / undefined / 配列 / primitive を排除、7 ケース網羅、#1283）                                                                                                                                                                                                           |
