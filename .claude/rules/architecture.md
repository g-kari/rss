...
| **プラットフォーム / 基盤** | storage / dev-log / serialize / concurrency / type guards         | `storage.ts` / `dev-log.ts` / `serialize-error.ts` / `serialize-async.ts` / `concurrency.ts` / `type-guards.ts` / `modal-focus.ts` / `context-menu-position.ts` / `popup-lock.ts`                                                                                                                                                                                                                                                                                                                                                                    |
...
| `article-filter-equality.spec.ts`               | `src/lib/article-filter-equality.ts` — `equalDigestLimitMap` / `equalStringMap` / `equalCompiledFilterMap` 構造的等価判定純粋関数 (旧 `useFilteredArticles.ts` inline 定義から canonical lib pattern に切り出し)                                                                                                                                           |
