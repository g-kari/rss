...
src/
  ...
  lib/
    ...
    export-json.ts           # ブックマーク・読書リスト記事を構造化 JSON (buildArticlesJson 純粋関数 + exportArticlesToJson) としてダウンロード — バックアップ/連携用
...
| **エクスポート / 変換**     | Markdown / Readwise / JSON / OPML 変換・Obsidian URI                        | `export-markdown.ts` / `export-readwise.ts` / `export-json.ts` / `html-to-markdown.ts` / `opml.ts` / `obsidian.ts` / `clip.ts`                                                                                                                                                                                                                                                                                                                                                                                                                       |
| **プラットフォーム / 基盤** | storage / dev-log / serialize / concurrency / type guards / 位置計算        | `storage.ts` / `dev-log.ts` / `serialize-error.ts` / `serialize-async.ts` / `concurrency.ts` / `type-guards.ts` / `modal-focus.ts` / `context-menu-position.ts` / `selection-popup-position.ts` / `popup-lock.ts`                                                                                                                                                                                                                                                                                                                                    |
...
| `export-json.spec.ts`                           | `src/lib/export-json.ts` — `buildArticlesJson` 純粋関数（mode 別 label / feedTitle 解決 / summary stripHtml+300字 clamp / author・publishedAt null / exportedAt ISO、15 ケース網羅、#1110）                                                                                                                                                                |
