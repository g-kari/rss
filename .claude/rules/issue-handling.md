...
**Why（このルールの背景）**: 2026-05-09 の #663 / #653 で発覚。前セッションで #653 に対し「現在記事の本文は取得しているはず」と AI が誤回答したが、本セッションでコードを精査したら #663 と同根のバグで実際は取得していなかった。質問者の "現在の記事の本文は取得していないの？" が正しい観察だったため、訂正コメントで誤回答を取り下げた。これを行わないと、Issue ログが「AI が嘘を言った」状態で残り、ユーザーの今後の判断材料を汚染する。
...
**Why（このルールの背景）**: 2026-05-09 の前セッションで `AutoReadController` の `prevPlayingRef` 同期バグについてサブエージェントが「`isAutoReadFinished` に enabled チェックがない」と誤分析した。実際にコードを読むと L30 で `if (!state.enabled) return false` のチェックは存在しており、別の場所（`articleId` 切替時の ref リセット）が真因だった。コードを直接確認していなかったら誤った修正を入れていた。
