...
**identifier 抽出 regex の前提**: `grep -oE` / `awk` で identifier (script 名 / 関数名 / 型名 / endpoint path segment 等) を抽出するときは **`[a-z][a-z0-9:_-]*`** (lowercase) + **`[A-Z][a-zA-Z0-9-]+`** (PascalCase) pattern を canonical とする。両 case とも `[a-z]` / `[A-Z]` クラス単独 (数字なし) は **数字混じり identifier を途中で truncate 抽出する false positive** を生む。lowercase 例: `pnpm run test:e2e` を `[a-z]` で抽出すると `pnpm run test:e` で truncate されて stale doc false positive。PascalCase 例: `A11yHelpers.tsx` を `[A-Z][a-zA-Z]+` で抽出すると `A` で truncate されて comparative sweep で「未文書化 drift」false positive 検出 (`src/components/` sweep 実例)。文字クラスに `0-9` を含めることで構造的に解消。該当する典型 identifier: lowercase = `test:e2e` / `v2` / `oauth2` / `vite-plus` / `react19` / `tailwind4` 等、PascalCase = `A11yHelpers` / `D1Database` / `OGP2` / `H1Heading` 等。kebab-case `:` / `_` / `-` も両 case で網羅。

**拡張子マッチング regex の前提 (`\.ts` で `.tsx` 部分 match 罠)**: source file 拡張子で identifier を抽出するとき、`\.ts` だけだと **`.tsx` の `.ts` 部分も match** して false positive を生む。markdown 内 `page.tsx` / `Component.tsx` 等が `.ts` regex で hit して「actual には `.tsx` のみ存在、doc に `.ts` 記載」の inverse drift と誤判定される。canonical pattern: **末尾固定 `\.ts$` + 別途 `\.tsx$` で 2 段 sweep**、もしくは **`\.tsx?$` で両方 hit させた後 caller の expected 拡張子で filter**。実例: `src/lib/*.ts` sweep で `architecture.md` 内 `app/demo/page.tsx` が `page.ts` として inverse drift hit → 末尾 `$` 固定で真の drift 0 件確認 (本前提宣言なしの初回 sweep で `page.ts` / `mock.ts` / `shortcuts.ts` 3 件 false positive 観測、grep collision で他 dir entry の混入と確認)。lowercase + PascalCase 同様 `\.ts` の末尾 anchoring は **拡張子 specificity の保証** として canonical。
