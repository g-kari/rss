...
Step 0 の一覧取得結果が空なら、Issue 対応だけでサイクルを終了せず、**必ず**自走開発を開始する。ユーザーから「新規機能を作れ」などの追加指示を待たず、**新規機能開発 / リファクタリング / パフォーマンス改善**から最も価値の高い 1 件を選んで実装する。
...
**候補の選び方**: 新規機能開発を第一候補とし、既存機能・README・未実装計画から要件を確定する。新規機能の根拠が弱い場合だけ、リファクタリング、パフォーマンス改善の順に実コードで最も根拠が強い候補を選ぶ。
...
### open Issue 0 件時の新規機能・大規模開発

open Issue が 0 件なら、ユーザーからの追加の明示許可を待たずに新規機能開発を開始する。新規機能を第一候補とし、次を理由にリファクタリングや微細なパフォーマンス改善へ差し替えてはならない。

- touch が 5 ファイルを超える
- UX 判断・API 設計・複数コンポーネントの変更を含む
- 複数 commit / 複数フェーズが必要になる
- 既存 pattern の単純延長ではない

この場合の進め方は次のとおり:

1. README、docs、既存 UI/API、型、関連テストを調査し、実装する機能の要件と完了条件を自分で確定する
2. `update_plan` で設計・実装・テスト・統合を分割し、必要なら複数 commit で進める
3. 新規 dependency / infra / R2・KV・D1 / CSP・認証変更を含めても、ユーザーへの追加確認で停止せず、既存のプロジェクト規則と安全要件に従って実装する
4. 主観的 UX は既存 UI の一貫性、アクセシビリティ、既存テスト、README の製品方針から合理的に決定する
5. 変更範囲が大きい場合も、各フェーズでテスト・型チェック・品質チェックを実行し、途中成果を commit する
6. 完了報告には、採用した機能仕様、変更範囲、未実装の残件、検証結果を明記する

なお、破壊的なデータ削除、秘密情報の公開、本番への直接変更、ライセンス違反は自走開発の対象外とする。
