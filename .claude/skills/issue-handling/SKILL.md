...
### ユーザーが新規機能・大規模開発を明示的に許可した場合

ユーザーが「新規機能を優先」「大きな新規機能を許可」「慎重さを除外」などと明示した場合、上記の保守的な自己制約を適用しない。具体的には、次を理由に新規機能をリファクタリングや微細なパフォーマンス改善へ差し替えてはならない。
...
3. 新規 dependency / infra / R2・KV・D1 / CSP・認証変更を含めても、ユーザーへの再確認で停止せず、既存のプロジェクト規則と安全要件に従って実装する
...
明示的な許可がない場合だけ、通常の自走条件に戻る。なお、破壊的なデータ削除、秘密情報の公開、本番への直接変更、ライセンス違反はこの許可の対象外とする。
