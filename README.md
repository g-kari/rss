...
| 💾 JSON バックアップ復元          | 検索条件・テーマ・記事状態・メモ・コレクションを復元     |
| 📊 読書統計                       | 年間ヒートマップ、フィード別集計、履歴 CSV・サマリー共有 |
| ⌨️ キーボードナビゲーション       | `j/k` 移動、`Ctrl+K` 検索、`,` 設定、`?` ヘルプ          |
...
- **JSON バックアップ復元**: 「設定 → インポート・エクスポート」から保存済み検索条件、記事状態、メモ、コレクションの JSON を取り込めます。表示設定のテーマプリセットは「設定 → 表示」から復元できます。
