...
### Phase 1: 外部フィード検索 + AI トピック抽出 (アプローチ 3 + 1 のトピック部分)

アプローチ3を動かすには「検索するトピック」が必要なため、アプローチ1のトピック抽出部分を先に実装。
LLM にフィード URL を提案させ、`discoverFeedUrl()` で実在確認する方式。

### Phase 2: AI 興味推定フィード提案の強化 (アプローチ 1)

トピック抽出を拡張し、複数ラウンドの LLM 呼び出しで精度向上。

### Phase 3: SharedFeedMeta 人気フィード (アプローチ 2)

`listAllFeedHashes()` と `buildFeedUserMap()` は既存。未購読フィルタとランキングを追加。

### Phase 4: 記事内リンク深度探索 (アプローチ 4)
...
| 操作 | ファイル                    | 内容                               |
| ---- | --------------------------- | ---------------------------------- |
| 変更 | `src/lib/recommendation.ts` | `generateAiSuggestions()` 精度向上 |
